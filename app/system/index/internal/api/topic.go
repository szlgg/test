...
// Index 主题首页
func (a *topicApi) Index(c *gin.Context) {
	var (
	...
	if PageData, getListRes, err := service.Content.GetList(req.ContentGetListInput); err != nil {
		...
		c.HTML(http.StatusOK, "topic/index.html", gin.H{
			"Title":        "主题",
...
// Detail 主题详情
func (a *topicApi) Detail(c *gin.Context) {
	// 获取内容id
	idStr := c.Param("id")
	// 转换成int类型
	...
	if getDetailRes, err := service.Content.GetDetail(id); err != nil {
		...
		c.HTML(http.StatusOK, "topic/detail.html", gin.H{
			"Title":      "详情",
			"Data":       getDetailRes,

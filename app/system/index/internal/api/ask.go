...
func (a *askApi) Index(c *gin.Context) {
	var (
	...
	if PageData, getListRes, err := service.Content.GetList(req.ContentGetListInput); err != nil {
		...
		c.HTML(http.StatusOK, "ask/index.html", gin.H{
			"Title":        "问答",
...
// Detail 问答详情
func (a *askApi) Detail(c *gin.Context) {
	// 获取内容id
	idStr := c.Param("id")
	// 转换成int类型
	...
	if getDetailRes, err := service.Content.GetDetail(id); err != nil {
		...
		c.HTML(http.StatusOK, "ask/detail.html", gin.H{
			"Title":      "详情",
			"Data":       getDetailRes,
